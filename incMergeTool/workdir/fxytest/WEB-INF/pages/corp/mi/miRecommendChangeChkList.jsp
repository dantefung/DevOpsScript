...
<head>
    <title><fmt:message key="mi.recommand.menu.check.title"/></title>
    <meta name="heading" content="<fmt:message key="mi.recommand.menu.check.title"/>"/>
</head>

<div class="searchBar">
	<form method="get" action="miRecommendChangeChks.jhtml" onsubmit="return validateSearch(this)">
...
<div class="contentBody">
	...
	<form id="miMemberApplyListTableForm" method="get" >
		<ec:table 
			...
			action="miRecommendChangeChks.jhtml"
					...
					<win:power powerCode="miRecommendChangeCheck">
						<c:choose>
							<c:when test="${(miMemberApplyVar.status==1 || miMemberApplyVar.status==3 || miMemberApplyVar.status==5 || miMemberApplyVar.status==7) && miMemberApplyVar.is_finished=='0' }">
								<a href="checkMiRecommendChange.jhtml?strAction=miRecommendChangeCheck&applyCode=${miMemberApplyVar.apply_code}" name="check"><fmt:message key="miMemberApalyCheck.check"/></a>
							</c:when>
							<c:otherwise>
								<a href="checkMiRecommendChange.jhtml?strAction=miRecommendChangeCheck&applyCode=${miMemberApplyVar.apply_code}" name="view"><fmt:message key="function.menu.view"/></a>
							</c:otherwise>
						</c:choose>
					</win:power>
