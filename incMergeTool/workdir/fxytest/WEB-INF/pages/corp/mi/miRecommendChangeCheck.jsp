...
<div class="contentBody" >
    <form:form commandName="miMemberApply" method="post" action="checkMiRecommendChange.jhtml" id="miMemberApplyForm" onsubmit="return validateForm(this)">
    ...
    <table class="detail">
	    ...
	    <c:if test="${not empty miMemberApplyDetailMap.MI_MEMBER}">
	    	...
	    	<c:if test="${not empty miMemberApplyDetailMap.MI_MEMBER.RECOMMEND_NO}">
	    	<tr>
	    		<th><fmt:message key="pl.plate.recommend"/>:</th>
	    		<td>${miMemberApplyDetailMap.MI_MEMBER.RECOMMEND_NO.OLD_VALUE }</td>
	    		<td>${miMemberApplyDetailMap.MI_MEMBER.RECOMMEND_NO.NEW_VALUE }</td>
	    	</tr>
	    	<tr>
	    		<th><fmt:message key="miMember.recommendName"/>:</th>
	    		<td>${miMemberApplyDetailMap.MI_MEMBER.RECOMMEND_NO.OLD_RECOMMEND_NAME}</td>
	    		<td>${miMemberApplyDetailMap.MI_MEMBER.RECOMMEND_NO.NEW_RECOMMEND_NAME}</td>
	    	</tr>
	    	</c:if>
	    </c:if>
    	<tr>
    	...
    	<tr>
    		<td colspan="3">
    			<div class="buttonBar">
    				...
    				<input type="hidden" name="strAction" value="miRecommendChangeCheck"/>
    				<input type="hidden" value="${form.token}" name="_form_uniq_id" />
    				<c:if test="${miMemberApply.status==3 }">
    					 <win:power powerCode="miRecommendChangeReturn"> 
    						<button type="submit" name="uncheck" onclick="return confirmReturn(this.form)"><fmt:message key="mi.return.applyFor"/></button>
    					 </win:power>
    					<win:power powerCode="miRecommendChangeCheck"> 
    						<button type="submit" name="check" onclick="return confirmAgree(this.form)"><fmt:message key="po.lpstatus.2"/></button>
    				    </win:power>
    				</c:if>
    				<button type="button" name="cancel" onclick="window.location='miRecommendChangeChks.jhtml?needReload=1'"><fmt:message key="operation.button.cancel"/></button>
                </div>
    		</td>
    	</tr>
    </table>
...
function confirmAgree(theForm){
	...
	if(confirm("<fmt:message key='mi.agree.sure'/>")){
		theForm.strAction.value="miRecommendChangeCheck";
		return true;
	}
	return false;
}
function confirmReturn(theForm){
	...
	if(confirm("<fmt:message key='mi.return.applyFor.sure'/>")){
		theForm.strAction.value="miRecommendChangeReturn";
		return true;
	}
	return false;
}
