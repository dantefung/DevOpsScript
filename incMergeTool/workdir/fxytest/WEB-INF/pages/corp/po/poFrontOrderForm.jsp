...
<div class="contentBody">
    <form:form commandName="poOrder" method="post" action="editPoFrontOrder.jhtml" onsubmit="return validateForm(this)" id="poOrderForm">
	    		...
	    		<td colspan="2">
	    			<ec:table 
						...
						<ec:row highlightRow="false">
							...
							<ec:column property="front_price" title="label.unit.price" styleClass="numberColumn">
								<fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.front_price }"/>
							</ec:column>
							<ec:column property="qty" title="购买数量" styleClass="numberColumn">
    		...
    		<td colspan="2">
    			<div class="buttonBar">
    				...
    				<c:if test="${not empty poOrder.orderId && poOrder.status!=100 }">
    					<win:power powerCode="deletePoFrontOrder">
    						<button type="submit" name="delete" onclick="bCancel=true;return confirmDelete(this.form,'PoFrontOrder')"><fmt:message key="operation.button.delete"/></button>
    					</win:power>
    				</c:if>
    				<button type="button" name="cancel" onclick="window.location='poFrontOrders.jhtml?needReload=1'"><fmt:message key="operation.button.cancel"/></button>
                </div>
    		</td>
    	</tr>
    </table>
