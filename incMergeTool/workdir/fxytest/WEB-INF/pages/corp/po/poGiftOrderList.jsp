...
<div class="searchBar">
	<form method="get" action="poGiftOrders.jhtml" onsubmit="return validateSearch(this)">
...
<div class="contentBody">
	<form name="memberOrderSearchForm" id="memberOrderSearchForm" action="editPoGiftOrder.jhtml" method="post" >
		<%@ include file="/common/messages.jsp" %>
		
		<div class="functionBar">
			<win:power powerCode="addPoGiftOrder">
				<a href="editPoGiftOrder.jhtml?strAction=addPoGiftOrder" name="add">
                    <fmt:message key="operation.button.add"/>
				</a>
				<span class="divider">&nbsp;</span>
			</win:power>
			<win:power powerCode="checkPoGiftOrder">
				<a href="javascript:multiCheckPoGiftOrder()" name="check">
					审核
				</a>
				<span class="divider">&nbsp;</span>
			</win:power>
			<win:power powerCode="multiDeleteGiftPoOrder">
				<a href="javascript:multiCancelPoGiftOrder()" name="multiDelete">
					作废
				</a>
				<span class="divider">&nbsp;</span>
			</win:power>
			...
			<ec:row>
				...
				<ec:column property="1" title="sysOperationLog.moduleName" sortable="false" style="width:15px;">
					<nobr>
						...
						<c:if test="${poOrder.status!=100 && poOrder.status!=999}">
							<!-- 未审核 -->
							<win:power powerCode="editPoGiftOrder">
								<a href="editPoGiftOrder.jhtml?strAction=editPoGiftOrder&orderId=${poOrder.order_id}" name="edit"><fmt:message key="button.edit"/></a>
							</win:power>
						</c:if>
					</nobr>
				</ec:column>
...
function multiCheckPoGiftOrder(){
	...
	if(confirm("<fmt:message key='确认审核所选择的单据'/>.")){
		...
		theForm.strAction.value="checkPoGiftOrder";
		
		showLoading();
		theForm.submit();
	}
}
function multiCancelPoGiftOrder(){
	var selectedValues = getCheckedBoxesValue(document.getElementsByName("chkOrderId"));
	if(selectedValues == null || selectedValues == ''){
		alert("<fmt:message key='请选择需要作废的单据'/>.");
		return;
	}

	if(confirm("<fmt:message key='确认作废所选择的单据'/>.")){
		
		var theForm = document.memberOrderSearchForm;
		theForm.selectedOrderIds.value = selectedValues;
		theForm.strAction.value="multiDeleteGiftPoOrder";
