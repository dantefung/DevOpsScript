...
<head>
    <title><fmt:message key="发货退回管理"/></title>
    <meta name="heading" content="<fmt:message key="发货退回管理"/>"/>
</head>
<%
...
<div class="contentBody">
    <form:form commandName="poShip" method="post" action="handlePoShipBack.jhtml" onsubmit="return validateForm(this)" id="poShipForm" name="poShipForm">
    ...
    <table class="detail">
    	...
    	<c:if test="${param.strAction=='checkPoShipBack' }">
	    	<tr>
	    		<th><fmt:message key="审核备注"/>:</th>
	    		<td colspan="3"><form:textarea path="checkRemark" id="checkRemark" cols="60" rows="6"/></td>
	    	</tr>
    	</c:if>
    	<c:if test="${param.strAction=='fiCheckPoShipBack' }">
	    	<tr>
	    		<th><fmt:message key="财务审核备注"/>:</th>
	    		<td colspan="3"><form:textarea path="fiCheckRemark" id="fiCheckRemark" cols="60" rows="6"/></td>
	    	</tr>
    	</c:if>
    	<c:if test="${param.strAction=='confirmPoShipBack' }">
	    	<tr>
	    		<th><fmt:message key="确认入库备注"/>:</th>
    		...
    		<td colspan="4">
    			<div class="buttonBar">
    				...
    				<win:power powerCode="${param.strAction }">
    					...
    					<c:if test="${poShip.checkStatus==1}">
    						<!-- 已审核 -->
    						<c:if test="${poShip.fiCheckStatus==0 }">
    							<!-- 未财务审核 -->
    							<c:if test="${param.strAction=='uncheckPoShipBack' }">
		    						<button type="submit" name="save" onclick="bCancel=false">取消审核</button>
		    					</c:if>
		    					<c:if test="${param.strAction=='fiCheckPoShipBack' }">
		    						<button type="submit" name="save" onclick="bCancel=false">财务审核</button>
		    					</c:if>
	    					</c:if>
	    					<c:if test="${poShip.confirmStatus==0 }">
    							<!-- 未入库 -->
		    					<c:if test="${param.strAction=='confirmPoShipBack' }">
		    						<button type="submit" name="save" onclick="bCancel=false">入库确认</button>
		    					</c:if>
	    					</c:if>
    					</c:if>
    				</win:power>
...
<c:if test="${param.strAction=='checkPoShipBack' ||  param.strAction=='fiCheckPoShipBack'}">
...
<c:if test="${param.strAction=='confirmPoShipBack' }">
	<c:set var="confirmMsg">确认入库</c:set>
</c:if>
