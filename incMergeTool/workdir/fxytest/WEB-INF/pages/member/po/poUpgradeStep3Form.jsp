...
<form method="get" action="poUpgradeStep3.jhtml" class="form-inline">
...
<form id="pmProductListTableForm" method="post" action="poUpgradeStep3.jhtml">
		...
		<tbody>
			<c:forEach items="${pmProducts}" var="pmProductVar">
				...
				<tr>
					...
					<td align="right"><fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.upgradePv }"/></td>
					<td align="center">
	 ...
	 <table class="detail" width="100%" border="0" cellspacing="0" cellpadding="0">
    		...
    		<td colspan="2">
    			<div class="buttonBar">
    				...
    				<button type="button" class="btn btn-default" name="previous" onclick="window.location='poUpgradeStep2.jhtml?strAction=${param.strAction }&newOrderKey=${newOrderKey }'"><fmt:message key="button.previous"/></button>
...
function validate() {
    ...
    if(arrayMoney[0] == arrayMoney[1]){
    	...
    	if(parseFloat($id("tpv").value) < parseFloat(arrayMoney[0])){
			...
			return;
		}else if (parseFloat($id("tpv").value) > parseFloat(arrayMoney[1])){
			alert("<fmt:message key="mi.register.1choose.chooseProduct.priceRange"/>");
			return;
		}else if(!isEmpty(mostMoney) && parseFloat($id("tpv").value) > parseFloat(mostMoney)){
			alert("<fmt:message key="mi.register.1choose.chooseProduct.priceMax"/>");
