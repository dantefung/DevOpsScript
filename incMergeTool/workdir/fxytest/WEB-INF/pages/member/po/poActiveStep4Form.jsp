...
<form method="get" action="poActiveStep4.jhtml" class="form-inline">
...
<form id="pmProductListTableForm" method="post" action="poActiveStep4.jhtml">
		...
		<tbody>
			<c:forEach items="${requestScope[pmProducts]}" var="pmProductVar">
				...
				<tr>
					...
					<td align="right"><fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.upgrade_price }"/></td>
					<td align="right"><fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.upgrade_pv }"/></td> 
					<td align="center">
						<c:choose>
							<c:when test="${not empty newOrderMap[cntMap2][_name] }">
								<%-- ${newOrderMap[cntMap2][_name]} --%>
								<input type="text" name="${pmProductVar.product_code }_quantity${i}" id="${pmProductVar.product_code }_quantity${i}" onchange="getTotal('${i}')" value="${newOrderMap[cntMap2][_name]}"/>
							</c:when>
							<c:otherwise>
								<%-- ${pmProductVar.ec_quantity*requestScope[productCnt]} --%>
								<input type="text" name="${pmProductVar.product_code }_quantity${i}" id="${pmProductVar.product_code }_quantity${i}" onchange="getTotal('${i}')" value="${pmProductVar.ec_quantity*requestScope[productCnt]}"/>
							</c:otherwise>
						</c:choose>
						...
						<input type="hidden" name="${pmProductVar.product_code }_unitPrice${i}" id="${pmProductVar.product_code }_unitPrice${i}" value="${pmProductVar.upgrade_price }"/>
						<input type="hidden" name="${pmProductVar.product_code }_unitPv${i}" id="${pmProductVar.product_code }_unitPv${i}" value="${pmProductVar.upgrade_pv }"/> 
	...
	<tr>
   		<td colspan="2">
   			<div class="buttonBar">
   				...
   				<button type="button" class="btn btn-default" name="previous" onclick="window.location='poActiveStep3.jhtml?strAction=${param.strAction }&newOrderKey=${newOrderKey }'"><fmt:message key="button.previous"/></button>
