...
<form id="pmProductListTableForm" method="post" action="poAgentOrderStep4.jhtml">
	 ...
	 <table class="table table-bordered table-striped">
		<thead>
			<tr>
				...
				<th><fmt:message key="折后单价"/></th>
				<th><fmt:message key="折后PV"/></th> --%>
		...
		<tbody>
			<c:forEach items="${requestScope[pmProducts]}" var="pmProductVar">
				...
				<c:set var="productCnt" value="productCnt${i }"/><!-- pmProductVar.ec_quantity -->
				...
				<tr>
					...
					<%-- <td align="right"><fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.return_price }"/></td>
					<td align="right"><fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.return_pv }"/></td> 
					<td align="right"><fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.return_price*discount }"/></td>
					<td align="right"><fmt:formatNumber pattern="###,##0.00" value="${pmProductVar.return_pv*discount }"/></td>  --%>
					<td align="center">
						...
						<input type="hidden" name="${pmProductVar.product_code }_unitPrice${i}" id="${pmProductVar.product_code }_unitPrice${i}" value="${pmProductVar.return_price*discount }"/>
						<input type="hidden" name="${pmProductVar.product_code }_unitPv${i}" id="${pmProductVar.product_code }_unitPv${i}" value="${pmProductVar.return_pv*discount }"/> 
	...
	<tr>
   		<td colspan="2">
   			<div class="buttonBar">
   				...
   				<button type="button" class="btn btn-default" name="previous" onclick="window.location='poAgentOrderStep3.jhtml?strAction=${param.strAction }&newOrderKey=${newOrderKey }'"><fmt:message key="button.previous"/></button>
