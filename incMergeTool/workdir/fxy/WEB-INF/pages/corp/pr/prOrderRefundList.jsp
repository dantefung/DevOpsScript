...
<div class="searchBar">
	<form method="get" action="prOrdersRefund.jhtml" onsubmit="return validateSearch(this)">
		...
		<dl>
			...
			<dd>
				...
				<input type="hidden" name="strAction" value="listPrOrdersRefund"/>
			</dd>
		</dl>
	</form>
</div>
		...
		<ec:table 
			...
			<ec:row>
				<ec:column property="1" title="sysOperationLog.moduleName" sortable="false">
					<c:choose>
						<c:when test="${prOrderVar.status==3}">
							<a href="editPrOrderRefund.jhtml?strAction=editPrOrderRefund&prId=${prOrderVar.pr_id}" name="check"><fmt:message key="退款确认"/></a>
						</c:when>
						<c:otherwise>
...
function multiDeletePrOrder(){
	var selectedValues=getCheckedBoxesValue(document.getElementsByName("refundPrId"));
	if(!selectedValues){
