...
<div class="contentBody">
	<form method="post" action="view_ic_message.jhtml" onsubmit="return validateDeleteMessage(this)" id="icMessageDeleteForm">
		<table class="detail" width="100%">
	    	...
	    	<tr>
	    		<td><fmt:message key="ic.lable.message.sender"/>：${oldIcMessage.senderCode }</td>
	    	</tr>
	    	<tr>
	    		<td>
	    	...
	    	<tr>
	    		<td>${fn:replace(oldIcMessage.content, vEnter, '<br>')}</td>
	    	</tr>
		    	...
		    	<tr>
		    		<td>
		    			<div class="buttonBar">
		    				...
		    				<input type="hidden" name="strAction" value="deleteIcMessage"/>
    ...
    <c:if test="${oldIcMessage.senderCode!=sessionScope.sessionLoginUser.userCode && empty icMessageTrash }">
	    ...
	    <table class="detail" width="100%">
	    	<tr>
	    		<td colspan="2" class="title"><fmt:message key="amMessage.replyContent"/></td>
	    	</tr>
	    	<c:if test="${empty replyMessage}">
		    	...
		    	<tr>
		    		<td colspan="2">
		    			<div class="buttonBar">
		    				<input type="hidden" name="messageId" value="${oldIcMessage.messageId }"/>
		    				<input type="hidden" name="strAction" value="icMessageReply"/>
		    				<input type="hidden" value="${form.token}" name="_form_uniq_id" />
		    				<win:power powerCode="icMessageReply">
	    	...
	    	<c:if test="${not empty replyMessage}">
	    		<tr>
		    		<td colspan="2" align="center"><font color="red"><fmt:message key="ic.lable.already.reply"/></font></td>
		    	</tr>
	    	</c:if>
	    </table>
	    </form>
...
function validateForm(theForm) {
	...
	if(theForm.title.value==""){
		alert("<fmt:message key='ic.reply.title.null.requie'/>");
		theForm.title.focus();
		return false;
	}
	if(theForm.content.value==""){
		alert("<fmt:message key='ic.reply.content.null.requie'/>");
