...
<head>
    <title><fmt:message key="mi.star.change.apply"/></title>
    <meta name="heading" content="<fmt:message key="mi.star.change.apply"/>"/>
</head>
...
<div class="contentBody">
    <form:form commandName="miMemberApply" method="post" action="miStarChangeForm.jhtml" onsubmit="return validateForm(this)" id="miForm">
    ...
    <table class="detail">
    	...
    	<tr>
   		...
   		<tr>
    		<th  class="required"><fmt:message key="mi.starType"/>:</th>
    		<td>
    		<win:list listCode="stars_level" name="star" defaultValue="" value="${param.star }" showBlankLine="true"/>
    		</td>
    	</tr>
    	<tr>
    	...
    	<tr>
    		<td colspan="2">
    			<div class="buttonBar">
    				...
    				<button type="button" name="cancel" onclick="window.location='miStarChanges.jhtml?needReload=1'"><fmt:message key="operation.button.cancel"/></button>
                </div>
    		</td>
    	</tr>
    </table>
...
function validateForm(theForm) {
	...
	if(isEmpty(theForm.star.value)){
		alert("<fmt:message key='mi.apply.star.null'/>！");
		theForm.star.focus();
		return false;
	}
...
function getMemberInfo(val) {
	...
	MiMemberAjax.getMiMemberInfo2(val,"1", function(data) {
		if (data != null) {
			document.getElementById("member_name").innerHTML ="<fmt:message key='poOrder.memberName'/>:"+ data.lastName+"<br/><fmt:message key='mi.star.old.leble'/>:"+data.star;
		} else {
