...
<div class="contentBody" >
    <form:form commandName="miMemberApply" method="post" action="checkMiLinkChange.jhtml" id="miMemberApplyForm" onsubmit="return validateForm(this)">
    ...
    <table class="detail" style="table-layout:fixed;" width='100%'>
    	<tr>
    	...
    	<tr>
    		<td colspan="3">
    			<div class="buttonBar">
    				...
    				<input type="hidden" name="strAction" value="miLinkChangeCheck"/>
    				<input type="hidden" value="${form.token}" name="_form_uniq_id" />
    				<c:if test="${miMemberApply.status==3 }">
    					 <win:power powerCode="miLinkChangeReturn"> 
    				...
    				<button type="button" name="cancel" onclick="window.location='miLinkChangeChks.jhtml?needReload=1'"><fmt:message key="operation.button.cancel"/></button>
                </div>
    		</td>
    	</tr>
    </table>
...
function confirmAgree(theForm){
	...
	if(confirm("<fmt:message key='mi.agree.sure'/>")){
		theForm.strAction.value="miLinkChangeCheck";
		return true;
	}
	return false;
}
function confirmReturn(theForm){
	...
	if(confirm("<fmt:message key='mi.return.applyFor.sure'/>")){
		theForm.strAction.value="miLinkChangeReturn";
		return true;
	}
	return false;
}
