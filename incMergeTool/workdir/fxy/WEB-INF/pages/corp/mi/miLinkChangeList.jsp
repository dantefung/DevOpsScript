...
<head>
    <title><fmt:message key="mi.link.menu.title"/></title>
    <meta name="heading" content="<fmt:message key="mi.link.menu.title"/>"/>
</head>
...
<div class="searchBar">
	<form method="get" action="miLinkChanges.jhtml" onsubmit="return validateSearch(this)">
...
<div class="contentBody">
	<c:if test="${empty param.strAction || param.strAction=='listMiMemberApplys'}">
		<form name="miMemberApplyFunctionForm" id="miMemberApplyFunctionForm" action="miLinkChangeForm.jhtml" method="post" >
			<%@ include file="/common/messages.jsp" %>
			<div class="functionBar">
				<win:power powerCode="addMiLinkChange">
					<a href="miLinkChangeForm.jhtml?strAction=addMiLinkChange" name="add"><fmt:message key="mi.link.change.apply"/></a>
		...
		<ec:table 
			...
			<ec:row>
				<ec:column property="1" title="sysOperationLog.moduleName" sortable="false">
					<win:power powerCode="viewMiLinkChange">
						<a href="viewMiLinkChange.jhtml?applyCode=${miMemberApplyVar.apply_code}" name="view"><fmt:message key="function.menu.view"/></a>
					</win:power>
					<c:if test="${empty param.strAction || param.strAction=='listMiRecommendChange'}">
						<win:power powerCode="addMiLinkChangeFile">
							<a href="editMiMemberApplyFile.jhtml?strAction=addMiLinkChangeFile&applyCode=${miMemberApplyVar.apply_code}" name="attachment"><fmt:message key="mi.attachment.manager"/></a>
						</win:power>
						<c:if test="${miMemberApplyVar.status == 0 }">
							<win:power powerCode="submitMiLinkChange">
						...
						<c:if test="${miMemberApplyVar.status == -1 || miMemberApplyVar.status == 0}">
							<win:power powerCode="deleteMiLinkChange">
	...
	<form action="miLinkChangeForm.jhtml" method="post" name="cancleForm" id="cancleForm">
		<input type="hidden" name="strAction" value="deleteMiLinkChange"/>
...
function subApply(applyCode){
	<win:power powerCode="submitMiLinkChange">
		if(confirm("<fmt:message key='mi.suresubmitm'/>")){
			...
			document.cancleForm.strAction.value = "submitMiLinkChange";
...
function cancleApply(applyCode){
	<win:power powerCode="deleteMiLinkChange">
		if(confirm("<fmt:message key='button.confirm.invalid'/>?")){
			document.cancleForm.applyCode.value = applyCode;
			document.cancleForm.strAction.value = "deleteMiLinkChange";
			document.cancleForm.submit();
		}
	</win:power>
}
