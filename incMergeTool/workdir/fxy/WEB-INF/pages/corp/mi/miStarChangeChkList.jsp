...
<div class="searchBar">
	<form method="get" action="miStarChangeChks.jhtml" onsubmit="return validateSearch(this)">
		...
		<dl>
			...
			<dd>
				...
				<input type="hidden" name="strAction" value="miStarChangeChks"/>
			</dd>
		</dl>
	</form>
</div>
			...
			<ec:row>
				<ec:column property="1" title="sysOperationLog.moduleName" sortable="false">
					<win:power powerCode="addMiStarChangeFile">
						<a href="editMiMemberApplyFile.jhtml?strAction=addMiStarChangeFile&applyCode=${miMemberApplyVar.apply_code}&isCheckPower=1" name="attachment"><fmt:message key="mi.attachment.manager"/></a>
					</win:power>
					<win:power powerCode="miStarChangeCheck">
						<c:choose>
							<c:when test="${(miMemberApplyVar.status==1 || miMemberApplyVar.status==3 || miMemberApplyVar.status==5 || miMemberApplyVar.status==7) && miMemberApplyVar.is_finished=='0' }">
								<a href="checkMiStarChange.jhtml?strAction=miStarChangeCheck&applyCode=${miMemberApplyVar.apply_code}" name="check"><fmt:message key="miMemberApalyCheck.check"/></a>
							</c:when>
							<c:otherwise>
								<a href="checkMiStarChange.jhtml?strAction=miStarChangeCheck&applyCode=${miMemberApplyVar.apply_code}" name="view"><fmt:message key="function.menu.view"/></a>
							</c:otherwise>
						</c:choose>
					</win:power>
				...
				<ec:column property="new_star" title="miMember.newStarNo" >
