...
<form method="get" action="miExchangeGoodsStep3.jhtml" class="form-inline">
...
<c:if test="${not empty productLimitMessages}">
	...
	<c:set var="_key" value="${newChangeKey }"/>
	<font color="red">${pvPrompt }</font>
<form id="pmProductListTableForm" method="post" action="miExchangeGoodsStep3.jhtml">
		...
		<tbody>
			<c:forEach items="${pmProducts}" var="pmProductVar">
				...
				<tr>
					...
					<td align="center">
						<input type="text" name="${pmProductVar.productId }_quantity" id="${pmProductVar.productId }_quantity" 
							<c:if test="${pmProductVar.productCode== bindProductNo }"> value="1" readonly="readonly" style="background-color: #ddd;" </c:if>
		    				<c:if test="${pmProductVar.productCode!= bindProductNo }"> value="${sessionScope[_key].cntMap[_name] }" </c:if>
							onchange="getTotal()"/>
		...
		<tr>
    		<td colspan="2">
    			<div class="buttonBar">
    				...
    				<button type="button" class="btn btn-default" name="previous" onclick="window.location='miExchangeGoodsStep2.jhtml?strAction=${strAction }&newChangeKey=${newChangeKey }'"><fmt:message key="button.previous"/></button>
