...
<form:form commandName="poShip" method="post" action="poOrderStep1.jhtml" onsubmit="return validateForm(this)" name="poShipForm" id="poShipForm">
    ...
    <table class="detail">
    	...
    	<tr class="shipType1TR" style="display:${shipType1Display};">
    		...
    		<td>
    			...
    			<select name="agentNo" id="agentNo" style="width:135px"><option><fmt:message key="list.please.select"/></option></select><!-- O2O店 -->
    			<br><div style="color: red;" ><b><fmt:message key="o2o.order.remark"/></b></div>
    		</td>
    	</tr>
    	<tr class="shipType1TR" style="display:${shipType1Display};">
		...
		<%-- <tr class="shipType1TR">
    		<th ><fmt:message key="po.o2o.no.lable"/>:</th>
    		<td><input name="agentNo" id="agentNo" value="${o2oNo }"/></td>
    	</tr> --%>
    	<tr>
    		<td colspan="2">
    			<div class="buttonBar">
...
$(document).ready(function(){
	...
	<c:if test="${not empty district && empty flagAreaCode }">
		...
		refreshO2Os("${o2oNo}","${district.regionId}","agentNo");
    ...
    $('#district').change(function(){
    	refreshO2Os("",$(this).children('option:selected').val(),"agentNo");
    });
  //如果分公司是中国，则可以选择国家
    if('${companyCode}'=="CN"){
...
//根据地区加载O2O
function refreshO2Os(selectedValue, regionId, targetObjId){
	dwr.util.removeAllOptions(targetObjId);
	MiMemberAjax.getO2OareaCodeToMap(regionId,'${companyCode}', function(data){
 		dwr.util.addOptions(targetObjId, data);
 		document.getElementById(targetObjId).value = selectedValue;
	});
}
